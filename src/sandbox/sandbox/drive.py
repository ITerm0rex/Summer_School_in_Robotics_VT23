...
class DifferentialDrive(Node):
    def __init__(self):
        ...
        self.cmd_subscription = self.create_subscription(
            Twist, "cmd", self.cmd_callback, 10
        )
    ...
    def cmd_callback(self, msg: Twist):
        ...
        left_speed = (
            dx - dr * self.b / 2
        ) / self.r  # 1.0 * self.dx - self.dr * self.w / 2.0
    ...
    def odom_callback(self):
        if self.left_encoder_value is not None and self.right_encoder_value is not None:
            ...
            if self.left_encoder_previous is None or self.left_encoder_previous is None:
                ...
                d_left = (
                    self.left_encoder_value - self.left_encoder_previous
                ) / self.ticks_per_meter
                d_right = (
                    self.right_encoder_value - self.right_encoder_previous
                ) / self.ticks_per_meter
